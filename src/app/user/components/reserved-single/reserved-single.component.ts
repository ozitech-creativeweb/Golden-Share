...
@Component({
  selector: 'app-reserved-single',
  templateUrl: './reserved-single.component.html',
  styleUrls: ['./reserved-single.component.scss']
})
export class ReservedSingleComponent implements OnInit {
  order: any;
  auth: any;
  currency: any;
  isLoading = false;
  ...
  private getDonation(id) {
    ...
    this.donationsService.order(id).subscribe(res => {
      if (res) {
        ...
        this.routingService.replace(['/user/reserved-orders']);
      }
