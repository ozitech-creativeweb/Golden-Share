...
import { PagesService } from 'src/app/data/services/administrator/pages.service';
import { UserBankAccountService } from 'src/app/data/services/user/user-bank-account.service';
...
export class AuctionComponent implements OnInit {
  ...
  submitBid(auctionID, auctionBalance, auctBank){
    this.form.get('auctionID').setValue(auctionID);
    this.form.get('bank').setValue(auctBank);
    ...
    if (auctBank !== this.bankUser.bank) {
      this.error = 'Oops! You can only bid on ' + this.bankUser.bank + ' coins';
      alert(this.error);
      this.scrollIntoView();
      this.isSubmitting = false;
      return;
        ...
        if (res && res.status === 'success') {
          ...
          if (res.data && res.data.length > 1) {
            ...
            this.routingService.replace([
              '/user/reserved-orders/' + res.data[0].id
            ]);
          } else {
